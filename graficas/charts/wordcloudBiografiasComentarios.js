 ...
 * Mode total: todos los perfiles | Mode sesgo: 200-4000 seguidores
 */
(function() {
  'use strict';
  ...
  function load(minFreq, mode) {
    ...
    fetch(url)
      ...
      .then((body) => {
        ...
        if (statsEl) {
          const label = m === 'total' ? 'Perfiles totales' : 'Perfiles 200-4000 seguidores';
          statsEl.textContent = `${label}: ${total.toLocaleString()} | Palabras: ${(list || []).length}`;
        }
        if (subtitleEl) {
          subtitleEl.textContent = m === 'total'
            ? 'Palabras extraídas de las biografías de todos los usuarios que comentaron'
            : 'Palabras extraídas de las biografías de usuarios con 200-4000 seguidores';
        }
        if (list && list.length) {
          render(list);
        } else {
